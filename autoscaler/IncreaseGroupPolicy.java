...
public class IncreaseGroupPolicy extends ScalePolicy { 
	...
	protected void policyTask(){
		...
		for (Instance webServer : webServers) {
			...
			if (webServers.size() < Constants.MAX_N_INSTANCES) { 
				...
				request = new GetMetricStatisticsRequest()
					...
					.withPeriod(Constants.INCR_POLICY_PERIOD)
		...
		System.out.println("Waiting: " + Constants.INCR_POOLING_PERIOD +" miliseconds for the next pooling.");
		getPolicyTimer(policyId).schedule(new IncreaseGroupPolicy(policyId), new Date(new Date().getTime() + Constants.INCR_POOLING_PERIOD));
	}
}
